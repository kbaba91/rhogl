...
GLRANSACWidget::GLRANSACWidget(QWidget *parent)
...
GLRANSACWidget::~GLRANSACWidget()
{
    cleanup();
}

QSize GLRANSACWidget::minimumSizeHint() const
{
    return QSize(50, 50);
}

QSize GLRANSACWidget::sizeHint() const
{
...
void GLRANSACWidget::setXRotation(int angle)
{
...
void GLRANSACWidget::setYRotation(int angle)
{
    qNormalizeAngle(angle);
    if (angle != m_yRot) {
        m_yRot = angle;
...
void GLRANSACWidget::setZRotation(int angle)
{
    qNormalizeAngle(angle);
    if (angle != m_zRot) {
        m_zRot = angle;
...
void GLRANSACWidget::initializeGL()
{
    ...
    connect(context(), &QOpenGLContext::aboutToBeDestroyed, this, &GLRANSACWidget::cleanup);
...
void GLRANSACWidget::setupVertexAttribs()
{
    m_shapeVbo.bind();
...
void GLRANSACWidget::setupInterestAttribs()
{
    m_interestVbo.bind();
    QOpenGLFunctions *f = QOpenGLContext::currentContext()->functions();
    f->glEnableVertexAttribArray(0);
...
void GLRANSACWidget::resizeGL(int w, int h)
{
    m_proj.setToIdentity();
    m_proj.perspective(45.0f, GLfloat(w) / h, 0.01f, 100.0f);
}

void GLRANSACWidget::mousePressEvent(QMouseEvent *event)
{
    m_lastPos = event->pos();
}

void GLRANSACWidget::mouseMoveEvent(QMouseEvent *event)
{
